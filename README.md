...
| 3 | 거래량 신호 (5일 관찰기간) | 최근 5거래일(당일 포함) 내 신호 발생 (아래) |

**당일 서지(surge)** — 거래량이 20일 평균의 1.5배 이상 5배 미만 + 당일 양봉
**지속 매집(acc)** — 최근 10일 중 매집일(가격↑ + 거래량 > 20일 평균)이 6일 이상

**관찰기간 (2026-08-21)** — 신호가 나온 종목은 이후 5거래일 동안 후보로 유지됩니다. 단, 유동성·RS 조건은 매일 다시 충족해야 합니다. 근거: 신규 신호 7,739건 지연 진입 백테스트에서 알파가 첫 2일 소폭 감쇠 후 7일까지 당일의 ~70% 유지. 효과: 후보 익일 생존율 45%→82%, 관찰군 ~100종목. `days_since_trigger`(경과일, 0=오늘 신규)와 `triggered_today` 컬럼으로 경과일을 표시합니다.
...
| `거래량5x+` | 거래량 20일 평균 5배 이상 + 양봉 — 어닝스/뉴스 갭 의심 (신호에서도 제외됨) |
...
`trigger_price` = 가장 최근 거래량 신호일 종가(발생가), `ext_from_trigger` = 현재가/발생가 − 1. 신호 이후 등락을 보여주는 참고값입니다.

**표시 전용이며 예측력은 없습니다** — 백테스트(71개 주간 스냅샷, 후보 관측치 7,564개)에서 신호 대비 등락률의 20d 초과수익 IC는 -0.03(t=-1.3, 비유의)이고 버킷별 성과도 비단조였습니다. 신호일 저가 이탈을 손절 기준으로 쓰는 가설도 기각됐습니다(이탈 그룹이 오히려 +2.05% vs 유지 +1.01%). 구 매수기준가(MA20/60×1.01 눌림 기준)는 폐기 — MA 괴리가 클수록 오히려 수익이 좋아(IC +0.06, t=+1.8) "눌림 대기" 전략을 지지하는 근거가 없었습니다.
