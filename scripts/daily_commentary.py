...
- 한국어 1~2문장, 120자 이내. 판정 사유(verdict_reason)를 반복하지 말고 실행 지침을 더하라:
  진입 검토 → 리스크 관리(경고 라벨, 포지션 크기, 분할 여부)와 확장도 해석. trigger_price는 신호가
  발생한 날의 종가(원점), ext_from_trigger는 원점 대비 등락률이다. 확장도가 낮으면 "신호 원점
  근처"라는 사실만 전달하라 — 눌림 대기를 권하지 마라(되돌림 대기 전략은 백테스트상 근거 없음).
  대기 → 기다리는 조건이 충족됐는지 판단할 기준(가격대, 날짜, 재트리거).
- 데이터 밖의 정성 정보를 알면 "⚠" 뒤에 한 구절로 덧붙여라 (예: "⚠ 비트코인 트레저리 회사 — 사실상 크립토 레버리지").
  이는 판정 변경이 아니라 참고 노트다. 모르면 붙이지 마라.
- 수치는 구체적으로 (가격, 배수, 날짜).

## market_comment 작성 규칙 — 개조식 브리핑
기관 데일리 노트의 톤을 지켜라: 권유형 어미("~바람직하다", "~하는 날이다") 금지,
각 라인은 관찰 → 해석 → 시사점 순으로 단정적으로 서술. 마크다운(**) 사용 금지.
아래 라벨의 라인들을 줄바꿈(\n)으로 구분해 작성하라 (라인당 1~2문장, 해당 사항 없는 라인은 생략):
국면: 시장 국면 판정과 그 견고함/취약점.
포지셔닝: 진입 검토군의 섹터·테마 구성과 주도력 해석, 쏠림이 있으면 노출 관리 원칙.
신호 품질: '## 집계' 블록 기반 — signal_type 구성(surge+acc 유무가 기대 알파 수준을 좌우).
  용어는 surge → "거래량 급증", acc → "지속 매집", surge+acc → "급증+매집"으로 표기하라.
  트리거 신선도(D+N)와 RSI 과열(75+) 간 긴장. 신선하지만 과열이면 즉시 진입과 눌림 대기 대상을 구분하라.
유효 베팅: 클러스터 압축('## 집계'의 클러스터 대표)과 섹터 쏠림을 감안한 실질 독립 베팅 수 추정과 그 함의.
변화: '## 추세 비교' 블록 기반 — 전일·1주·1개월 대비 시장 폭, RS 분포, 주도 섹터가 어느 방향으로
  움직이는 중인지 해석하라. 수치 나열이 아니라 방향성과 그 함의(개선/악화/횡보, 로테이션)를 서술.
  블록에 없는 수치를 지어내지 마라.
이벤트: 어닝스 대기 등 일정 리스크.
...
def build_market_summary(df: pd.DataFrame) -> str:
    ...
    lines.append(f"트리거 신선도: D+0~2 {int((fresh <= 2).sum())}개 / D+3 이상 {int((fresh >= 3).sum())}개")
    reps = []
